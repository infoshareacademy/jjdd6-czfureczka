...
public class Upload {
    ...
    @Inject
    StopTimesWithDate stopTimesWithDate;
        ...
        if (validation.validationOfStopName(stopDesc)) {
            if (listRoute.checkRouteId(routeId)) {
                ...
                if (trip.checkTripId(tripId)) {
                    return cropDateFromTime(getFullTimetable(stopDesc, Integer.valueOf(tripId)));
                }
            }
        }
    ...
    private List<String> getFullTimetable(String stopDesc, Integer tripId) {
        ...
        List<String> result = stopTimesWithDate.getStopTimes().stream()
